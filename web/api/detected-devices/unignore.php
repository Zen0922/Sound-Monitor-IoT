...
function update_unignore_flag(PDO $pdo, string $deviceBtAddr): void
{
    $stmt = $pdo->prepare(
        '
    ...
    if ($stmt->rowCount() < 1) {
        throw new OutOfBoundsException('無視中の機器が見つかりませんでした');
    }
}
...
try {
    ...
    update_unignore_flag($pdo, $deviceBtAddr);
    ...
    error_log('[api/detected-devices/unignore] ' . $e->getMessage());

    json_error('無視解除に失敗しました', 500, [
        'error' => $e->getMessage(),
    ]);
}